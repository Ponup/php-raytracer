...
class Ray {
	...
	public function __construct(\glm\vec3 $origin, \glm\vec3 $direction) {
		$this->origin = $origin;
	...
	public static function createPrime(int $x, int $y, Scene $scene) : Ray {
		...
		/**
 		 ...
 		 */
		$fovAdjustment = tan(\glm\radians($scene->fov) / 2.0);
		...
		$ray = new Ray(
			...
			($direction = new \glm\vec3($sensorX, $sensorY, -1.0))->normalize()
		);
		return $ray;
	}
}
